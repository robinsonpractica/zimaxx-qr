...
export interface User { id: string; email: string; display_name: string; status: "active" | "disabled"; role: UserRole; password_salt: string; password_hash: string; }
...
export async function findUserByEmail(db: D1Database, email: string): Promise<User | null> {
  return db.prepare("SELECT id,email,display_name,status,role,password_salt,password_hash FROM users WHERE email=? COLLATE NOCASE").bind(email.trim()).first<User>();
}
...
export async function listCodes(db: D1Database): Promise<CodeRecord[]> {
  const { results } = await db.prepare(`SELECT c.*,r.destination_url,COUNT(s.id) AS total_scans FROM codes c JOIN redirect_rules r ON r.code_id=c.id AND r.valid_to IS NULL LEFT JOIN scan_events s ON s.code_id=c.id WHERE c.status!='archived' GROUP BY c.id ORDER BY c.updated_at DESC`).all<CodeRecord>();
  return results;
}

export async function getCode(db: D1Database, id: string): Promise<CodeRecord | null> {
  return db.prepare(`SELECT c.*,r.destination_url,COUNT(s.id) AS total_scans FROM codes c JOIN redirect_rules r ON r.code_id=c.id AND r.valid_to IS NULL LEFT JOIN scan_events s ON s.code_id=c.id WHERE c.id=? GROUP BY c.id`).bind(id).first<CodeRecord>();
}
...
export async function updateCode(db: D1Database, current: CodeRecord, input: { actorId: string; name: string; destination: string; foreground: string; background: string; errorCorrection: ErrorCorrection; expectedVersion: number; idempotencyKey: string; }) {
  const now = new Date().toISOString(); const changedDestination = current.destination_url !== input.destination;
  const statements = [
    db.prepare("UPDATE codes SET name=?,foreground=?,background=?,error_correction=?,version=version+1,updated_at=? WHERE id=? AND version=? AND status!='archived'").bind(input.name,input.foreground,input.background,input.errorCorrection,now,current.id,input.expectedVersion),
  ];
  if (changedDestination) {
    ...
    statements.push(db.prepare("INSERT INTO redirect_rules(id,code_id,revision,destination_url,valid_from,changed_by,created_at) VALUES(?,?,?,?,?,?,?)").bind(newId("rr"),current.id,(revision?.revision??0)+1,input.destination,now,input.actorId,now));
  }
  statements.push(db.prepare("INSERT INTO audit_log(id,owner_id,code_id,action,idempotency_key,metadata_json,created_at) VALUES(?,?,?,'code.updated',?,?,?)").bind(newId("aud"),input.actorId,current.id,input.idempotencyKey,JSON.stringify({destinationChanged:changedDestination}),now));
...
export async function setCodeStatus(db: D1Database, actorId: string, id: string, status: CodeStatus, idempotencyKey: string) {
  const now = new Date().toISOString();
  await db.batch([
    db.prepare("UPDATE codes SET status=?,updated_at=?,version=version+1 WHERE id=? AND status!='archived'").bind(status,now,id),
    db.prepare("INSERT OR IGNORE INTO audit_log(id,owner_id,code_id,action,idempotency_key,metadata_json,created_at) VALUES(?,?,?,?,?,?,?)").bind(newId("aud"),actorId,id,`code.${status}`,idempotencyKey,"{}",now),
  ]);
}
...
export async function analytics(db: D1Database) {
  const { results } = await db.prepare(`SELECT c.id,c.name,c.slug,se.occurred_date,COUNT(se.id) scans FROM codes c LEFT JOIN scan_events se ON se.code_id=c.id AND se.occurred_date>=date('now','-29 days') WHERE c.status!='archived' GROUP BY c.id,se.occurred_date ORDER BY se.occurred_date`).all<{id:string;name:string;slug:string;occurred_date:string|null;scans:number}>();
  return results;
}
